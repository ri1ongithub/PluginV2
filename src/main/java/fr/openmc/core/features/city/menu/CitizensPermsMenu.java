...
public class CitizensPermsMenu {
    public static void openBookFor(Player sender, UUID player) {
        ...
        if (hisCity == null) {
            MessagesManager.sendMessageType(sender, "Ce joueur n'habite aucune ville", Prefix.CITY, MessageType.ERROR, false);
            return;
        }
        ...
        if (!Objects.equals(hisCity.getUUID(), city.getUUID())) {
            MessagesManager.sendMessageType(sender, "Ce joueur n'habite pas dans ta ville", Prefix.CITY, MessageType.ERROR, false);
            return;
        }

        if (!city.hasPermission(sender.getUniqueId(), CPermission.PERMS)) {
            MessagesManager.sendMessageType(sender, "Tu n'as pas la permission d'ouvrir ce menu", Prefix.CITY, MessageType.ERROR, false);
            return;
        }
